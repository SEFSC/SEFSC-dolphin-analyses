...
intc <- read.csv("data/FINAL_files/CAR_rep_unrep_TomFormat.csv")
inth <- read.csv("data/FINAL_files/Intl_highseas_TomFormat.csv")
intn <- read.csv("data/FINAL_files/NED_Intl_TomFormat.csv")
...
d <- rbind(intc, inth, intn, com, rec)

# check for zeros and NAs
#hist(d$Catch_lbs)
table(d$Catch_lbs == 0)
table(is.na(d$Catch_lbs))
apply(d, 2, function (x) table(is.na(x)))
...
barplot(tab, beside = T, col = 2:6, legend = rownames(tab),
...
matplot(rownames(tab), tab, 
        ...
        ylab = "total catch (millions of pounds)")
legend("top", lwd = 2, lty = c(rep(1, 3), 2, 2), bty = "n", 
...
```{r}
#| echo: true
write.csv(d, file = "data/FINAL_files/complete_catch_dataset_09052025.csv", row.names = FALSE)
```
