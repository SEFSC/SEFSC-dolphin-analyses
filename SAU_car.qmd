...
CANqrt <- CANqrt[, !is.na(colSums(CANqrt))] # remove NA columns from CAN data

# plot the parsed out data by year-quarter
yrs <- sort(rep(as.numeric(colnames(CARrep)), 4))
qrt <- rep(1:4, length(yrs)/4)
yrs1 <- sort(rep(as.numeric(colnames(CANqrt)), 4))
...
matplot(yrs + qrt/4, cbind(matrix(CARrep), matrix(CARunr))/10^6, lty = 1, lwd = 2, 
        ...
        xlab = "", ylab = "total catch (millions of pounds)")
lines(yrs1 + qrt1/4, as.numeric(matrix(CANqrt))/10^6, lwd = 2, col = 4)
legend("topleft", c("Caribbean reported landings", "Caribbean unreported landings", "Canadian reported landings"),        lwd = 2, col = 2:4, bty = "n")
...
write.csv(findat, file = "data/outputs/CAR_rep_unrep_TomFormat.csv")

#barplot(tapply(findat$Catch_lbs, list(findat$Quarter), sum, na.rm = T), col = rainbow(4))
