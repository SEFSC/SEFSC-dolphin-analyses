...
## Partition landings by area and sector
...
# output discarding rates for later use
...
names(findat)[3:10] == names(dismat)[2:9]  # check columns are the same

findat <- findat[which(findat$year < 2025), ]
findat_disc <- findat
...
round(dismat[5, 2:9], 3) == round(out[2, ], 3)
round(dismat[nrow(dismat), 2:9], 3) == round(out[nrow(out), ], 3)

# calculate % of post-release dead discards by area
round((colSums(findat_disc[, 3:10]) / colSums(findat[, 3:10]) - 1) * 100, 2)
# calculate recent % of post-release dead discards by area
round((colSums(findat_disc[149:160, 3:10]) / colSums(findat[149:160, 3:10]) - 1) * 100, 2)
# calculate overall % of post-release dead discards
round((sum(findat_disc[, 3:10]) / sum(findat[, 3:10]) - 1) * 100, 2)
# calculate recent % of post-release dead discards
round((sum(findat_disc[149:160, 3:10]) / sum(findat[149:160, 3:10]) - 1) * 100, 2)
