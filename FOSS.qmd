...
There are some issues with the way that the FOSS system splits out Gulf and Atlantic landings by Monroe County, which is on the border between the two jurisdictions. We received data from Mike Larkin (SERO) which has the correct split out of data for Monroe County between the Gulf and Atlantic. We compare the total recreational landings for the Gulf and Atlantic between the FOSS data and the updated data. The numbers are very close; the average difference from the FOSS data is 2%.
...
We replace the FOSS values for Gulf and South Atlantic recreational landings with these new values and re-plot the full data set. We can also plot the landings as a line graph so that the relative magnitudes of landings across region and sector are more easily compared.
