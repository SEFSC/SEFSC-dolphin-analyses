...
Note there are few (a fraction of a percent) discards and unreported landings -- nearly all of the high seas landings appear as reported and landed fish.
...
percatch[, , 7] # these are the percentages for the NED (Atlantic Northwest)
...
for (i in 1:ncol(percatch))  { 
  NED[, i] <- percatch[, i, 7] * tab[i, 1]
  NCA[, i] <- percatch[, i, 1] * tab[i, 2]
  }
...
are <- rep("NED", length(yrs))
fin1 <- data.frame(cbind(yrs, qrt, flt, are, matrix(NED)))
are <- rep("NCA", length(yrs))
fin2 <- data.frame(cbind(yrs, qrt, flt, are, matrix(NCA)))
...
write.csv(findat, file = "data/outputs/Intl_highseas_TomFormat.csv")

tab <- tapply(findat$Catch_lbs, list(findat$Quarter, findat$Area), sum, na.rm = T)
#barplot(tab, beside = T, col = rainbow(4))

matplot(yrs + qrt/4, cbind(as.numeric(fin1$V5), as.numeric(fin2$V5))/10^6, lty = 1, lwd = 2, 
